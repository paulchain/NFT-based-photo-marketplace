    ...
    try {
      ...
      if (!isProd) {
        ...
        if (PhotoNFTData.networks) {
          ...
          if (deployedNetwork) {
            instancePhotoNFTData = new web3.eth.Contract(PhotoNFTData.abi, deployedNetwork && deployedNetwork.address)
    ...
    return (
      <div className={styles.contracts}>
        ...
        <Container>
          <Row>
            {allPhotos
              .filter((photo) => currentAccount != photo.ownerAddress && photo.status == 'Open')
              .map((photo, key) => {
                return (
                  <Col md={3}>
                    <div key={key} className="">
                      <div className={styles.widgets}>
                        <Card width={'360px'} maxWidth={'360px'} mx={'auto'} my={5} p={20} borderColor={'#E8E8E8'}>
                          <Image
                          ...
                          <Button size={'medium'} width={1} value={photo.photoNFT} onClick={this.buyPhotoNFT}>
                            {' '}
                            Buy{' '}
                          </Button>
